...
use Apitis\Checkers\Contexts\Game\Domain\Event\MovePerformed;
...
use Apitis\Checkers\Contexts\Game\Domain\Collection\Exception\NoPieceOnFieldException;
use Apitis\Checkers\Contexts\Game\Domain\Policy\Rules;
use Apitis\Checkers\Contexts\Game\Domain\ValueObject\CapturedPiece;
use Apitis\Checkers\Contexts\Game\Domain\ValueObject\Field;
use Apitis\Checkers\Contexts\Game\Domain\ValueObject\Move;
use Apitis\Checkers\Contexts\Game\Domain\ValueObject\Piece;
...
class Board extends SimpleEventSourcedEntity
{
    ...
    private function getCorner(Direction $direction)
    {
        switch($direction) {
            ...
            case Direction::SOUTHEAST(): {
                return new CartesianCoordinates($this->boardSize,1);
            }
            case Direction::NORTHWEST(): {
                return new CartesianCoordinates(1,$this->boardSize);
            }
            case Direction::NORTHEAST(): {
                return new CartesianCoordinates($this->boardSize,$this->boardSize);
            }
        }
    ...
    {
        return ($coordinates->getX() > 0 && $coordinates->getX() <= $this->boardSize &&
            $coordinates->getY() > 0 && $coordinates->getY() <= $this->boardSize);
    }
